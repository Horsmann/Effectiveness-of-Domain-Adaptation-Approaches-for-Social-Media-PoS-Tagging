...
public class Rehbein2013TweetCorpusReader extends
	...
	public void getNext(JCas aJCas) throws IOException, CollectionException {
		...
		md.setDocumentTitle("Rehbein2013");
		md.setDocumentId("" + new Random().nextInt());
	...
	private boolean skipLine(String readLine) {
		return readLine.startsWith("<tweet ") || readLine.startsWith("<text")
				|| readLine.startsWith("</text") || !readLine.startsWith("<");
	}

	private String[] extractInfoFromXML(String aReadLine) {
		String LEMMA_PATTERN = "lemma=\"";
		String TYPE_PATTERN = "type=\"";
		String WORD_PATTERN = ">";

		int lemStart = aReadLine.indexOf(LEMMA_PATTERN);
		int lemEnd = aReadLine.indexOf("\"", lemStart + LEMMA_PATTERN.length());
		String lemma = aReadLine.substring(lemStart + LEMMA_PATTERN.length(),
				lemEnd);

		int posStart = aReadLine.indexOf(TYPE_PATTERN);
		int posEnd = aReadLine.indexOf("\"", posStart + TYPE_PATTERN.length());
		String pos = aReadLine.substring(posStart + TYPE_PATTERN.length(),
				posEnd);

		int wordStart = aReadLine.indexOf(WORD_PATTERN);
		int wordEnd = aReadLine.indexOf("</w>");
		String word = aReadLine.substring(wordStart + WORD_PATTERN.length(),
				wordEnd);
