 ...
 ******************************************************************************/
package de.unidue.langTech.converter.penn;
...
public class PennTreebankChunkedReader extends
	...
	/**
	 * Use this part-of-speech tag set to use to resolve the tag set mapping
	 * instead of using the tag set defined as part of the model meta data. This
	 * can be useful if a custom model is specified which does not have such
	 * meta data, or it can be used in readers.
	 */
	...
	public static final String PARAM_USE_COARSE_GRAINED = "PARAM_USE_COARSE_GRAINED";
	@ConfigurationParameter(name = PARAM_USE_COARSE_GRAINED, mandatory = false)
	private boolean useCoarseGrainedPosTags;
	...
	public void getNext(JCas aJCas) throws IOException, CollectionException {
		...
		try {
			...
			while ((readLine = br.readLine()) != null) {
				...
				for (String twt : tokenWithTags) {
					...
					// two words might be joined by a forward slash, the same
					// symbol
					// which separates token from part of speech tag. The
					// word-join
					...
					// speech tags. We take the first one named and ignore the
					// other
					// ones
					...
					// misspelled in the source material. 'The students
					// dormitory'
					// should have used an apostrophe to mark a possessive case
					// for
					// the word <code>students'</code>. The
					// misspelling lead to a plural noun pos-tag although the
					// possessive
					// tag would have been correct from the view point of
					// intention.
					// We chose the incorrect(!) part of speech tag here to
					// avoid
		...
		addTCAnnotations(aJCas);
	}

	private void addTCAnnotations(JCas aJCas) {
		Collection<Sentence> sentences = JCasUtil.select(aJCas, Sentence.class);
		for (Sentence s : sentences) {
			TextClassificationSequence sec = new TextClassificationSequence(
					aJCas, s.getBegin(), s.getEnd());
			sec.addToIndexes();

			List<Token> tokens = JCasUtil.selectCovered(aJCas, Token.class,
					s.getBegin(), s.getEnd());
			for (Token t : tokens) {
				TextClassificationUnit tu = new TextClassificationUnit(aJCas,
						t.getBegin(), t.getEnd());
				tu.setId(randomId++);
				tu.setSuffix(t.getCoveredText());
				tu.addToIndexes();

				TextClassificationOutcome to = new TextClassificationOutcome(
						aJCas, t.getBegin(), t.getEnd());
				to.setOutcome(getOutcome(t));
				to.addToIndexes();
			}
		}
	}

	private String getOutcome(Token aT) {
		POS p = aT.getPos();

		if (useCoarseGrainedPosTags) {
			return p.getClass().getSimpleName();
		}
		return p.getPosValue();
	}

	private void annotateChunks(JCas aJCas, List<int[]> aChunkStartEndIdx) {
		List<Token> tokens = new ArrayList<Token>(JCasUtil.select(aJCas,
				Token.class));
	...
	private String ifWordIsMisspelledSelectTagThatFitsTheMisspelledWord(
			String aTag) {
		// replace by whitespace and trim the one at the beginning away, the
		// remaining one are our
		// split points
		if (aTag.contains("^")) {
	...
	private String annotateSenenceTokenPosTypes(JCas aJCas,
			List<String> aTokens, List<String> aTags) {
	...
	private void annotateTokenWithTag(JCas aJCas, String aToken, String aTag,
			int aCurrPosInText) {
		// Token
		Token token = new Token(aJCas, aCurrPosInText, aToken.length()
				+ aCurrPosInText);
		token.addToIndexes();
