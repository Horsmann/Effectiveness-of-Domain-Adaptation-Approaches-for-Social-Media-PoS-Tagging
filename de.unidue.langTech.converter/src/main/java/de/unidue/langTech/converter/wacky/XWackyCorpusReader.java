...
public class XWackyCorpusReader
    ...
    implements TCReaderSequence
{
    ...
    @ConfigurationParameter(name = PARAM_READ_LEMMA, mandatory = false, defaultValue = "true")
    private boolean lemma;

    public static final String PARAM_READ_LIMIT = "PARAM_READ_LIMIT";
    @ConfigurationParameter(name = PARAM_READ_LIMIT, mandatory = false)
    private int limit;
        ...
        throws ResourceInitializationException
    {
        ...
        try {
            for (Resource r : getResources()) {
                ...
                if (r.getResource().getFile().getName().endsWith(".gz")) {
        ...
        throws IOException, CollectionException
    {
        ...
        md.setDocumentTitle("WackyReader");
        ...
        for (int i = 0; i < sentence.size(); i++) {
            ...
            String lemmaText = entry[1];
            String posText = entry[2];
            ...
            if (lemma) {
                Lemma lemma = new Lemma(aJCas, tokStart, tokEnd);
                lemma.setValue(lemmaText);
                lemma.addToIndexes();
        ...
        if (countTokens % 10000 == 0) {
            LogFactory.getLog(getClass().getName()).warn(
                    String.format("Read tokens: %10d", countTokens));
        }
    }
        ...
        throws IOException, CollectionException
    {
        ...
        while ((readLine = br.readLine()) != null) {
            ...
            String[] data = extractInfo(readLine);
    ...
    {
        return readLine.startsWith("<sentence") || readLine.startsWith("<text")
                || readLine.startsWith("</text") || readLine.startsWith("</sentence")
                || readLine.startsWith("<year>") || readLine.startsWith("<source=")
                || readLine.startsWith("<error=") || readLine.equals("<s>");
    }

    private String[] extractInfo(String aReadLine)
    {
        String[] split = aReadLine.split("\t");

        if (split.length < 3) {
            System.err.println("Error:" + aReadLine);
            return new String[] { "", "", "" };
        }
        ...
        if (lemma.contains("|") && lemma.split("\\|").length > 1) {
            lemma = lemma.split("\\|")[0];
        }

        String[] data = { word, lemma, pos };
